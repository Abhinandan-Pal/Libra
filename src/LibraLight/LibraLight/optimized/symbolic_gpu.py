...
def relu_compute_GPU(d_lbs, d_ubs, d_symb_layer, d_active_pattern,d_l1_lb,d_l1_ub,d_if_activation):
    @cuda.jit
    def relu_compute_helper(lbs, ubs, symb_layer, active_pattern,if_activation):
        init_id,id = cuda.grid(2)
...
def oneOutput(d_affine,d_symb,if_activation,d_l1_lb,d_l1_ub,outNodes,inv_var_index,sensitive,print_mode):
    ...
    for out1 in outNodes:
        ...
        layer = len(d_affine)
        d_ineq_lte, d_ineq_gte = back_propagate_GPU1(d_ln_coeff_lte, d_ln_coeff_gte, d_affine, d_symb, layer,
                                                     if_activation)
...
def detailedPrintCondense( d_affine, d_symb, d_active_pattern, d_l1_lb, d_l1_ub, if_activation,d_if_activation, var_index, inv_var_index,dims):
    ...
    for i in range(1, len(d_affine)):
        ...
        if (any(if_activation[i])):
            d_lbs, d_ubs = get_bounds_GPU(d_ineq_lte, d_ineq_gte, d_l1_lb, d_l1_ub)
            relu_compute_GPU(d_lbs, d_ubs, d_symb[:, i], d_active_pattern[:, i], d_l1_lb, d_l1_ub,d_if_activation[i])
        ...
        for j in range(1, dims[i]):
            ...
            print(f" eq (LB,UB): {commons.get_bounds_single(ineq_lte[init_id], ineq_gte[init_id], j, l1_lb[init_id], l1_ub[init_id])}")
        if (any(if_activation[i])):  # assuming if first node in a layer has activation then all do
            ...
            for j in range(1, dims[i]):
                ...
                if (symb[init_id][i][j][0] == 0.0):
                    print(f"Relu eq (LB,UB): {commons.get_bounds_single(ineq_lte[init_id], ineq_gte[init_id], j, l1_lb[init_id], l1_ub[init_id])}")
                else:
...
def miniPrintCondense(d_affine,d_symb,d_active_pattern,d_l1_lb,d_l1_ub,if_activation,d_if_activation,inv_var_index,sensitive,dims):
    l1_ub = cp.asnumpy(d_l1_ub)
    l1_lb = cp.asnumpy(d_l1_lb)
    #init_id = 0  # len(d_l1_ub) -1
    #print(f"init_id-> {init_id}; lbs -> {d_l1_lb[init_id]}; ubs -> {d_l1_ub[init_id]}")
    for i in range(1, len(d_affine)):
        ...
        for init_id in range(len(ineq_lte)):
            ...
            if (any(if_activation[i])):  # assuming if first node in a layer has activation then all do
                for j in range(1, dims[i]):
                    if (symb[init_id][i][j][0] == 0.0):
                        print(f"Relu{inv_var_index[(i,j)]} eq (LB,UB): {commons.get_bounds_single(ineq_lte[init_id], ineq_gte[init_id], j, l1_lb[init_id], l1_ub[init_id])}")
                    else:
                        print(f"Relu{inv_var_index[(i,j)]} eq (LB,UB): ({symb[init_id][i][j][2]},{symb[init_id][i][j][1]})")
            else:
                for j in range(1, dims[i]):
                    print(f"{inv_var_index[(i,j)]} eq (LB,UB): {commons.get_bounds_single(ineq_lte[init_id], ineq_gte[init_id], j,l1_lb[init_id],l1_ub[init_id])}")

def noPrintCondense(d_affine, d_symb, i, if_activation,d_if_activation, d_active_pattern,d_l1_lb,d_l1_ub):
    for i in range(1, len(d_affine)):
        d_ineq_lte, d_ineq_gte = back_propagate_GPU(d_affine, d_symb, i, if_activation)
        if (any(if_activation[i])):
            d_lbs, d_ubs = get_bounds_GPU(d_ineq_lte, d_ineq_gte, d_l1_lb, d_l1_ub)
...
def analyze(netGPU,l1_lbL,l1_ubL,percent,L,print_mode):
    ...
    for i in range(len(l1_ub_list)):
        ...
        if(print_mode == 1):
            detailedPrintCondense(d_affine, d_symb, d_active_pattern, d_l1_lb, d_l1_ub, if_activation,d_if_activation, var_index,inv_var_index,dims)
        elif (print_mode == 2):
            miniPrintCondense(d_affine, d_symb, d_active_pattern, d_l1_lb, d_l1_ub, if_activation,d_if_activation,inv_var_index,sensitive,dims)
        elif (print_mode == 3):
            noPrintCondense(d_affine, d_symb, i, if_activation,d_if_activation, d_active_pattern, d_l1_lb, d_l1_ub)

        outcome = oneOutput(d_affine, d_symb, if_activation, d_l1_lb, d_l1_ub, outNodes, inv_var_index,sensitive,print_mode)
