...
def relu_compute_GPU(d_lbs, d_ubs, d_relu_layer, d_active_pattern,d_l1_lb,d_l1_ub):
    @cuda.jit
...
def back_propagate_GPU(d_affine, d_relu, layer: int, if_activation, d_active_pattern,d_l1_lb,d_l1_ub):
    ...
    '''
    print(f"{ln_shape}")
    d_ln_coeff_lte = cp.zeros(ln_shape).astype('float32')
    d_ln_coeff_gte = cp.zeros(ln_shape).astype('float32')
    d_ln_coeff_lte[0] = d_affine[layer].copy().astype('float32')  # Need to create copies
...
def detailedPrintCondense(d_affine,d_relu,d_active_pattern,d_l1_lb,d_l1_ub,if_activation,relu,var_index,inv_var_index,l1_lb,l1_ub):
    ...
    for i in range(1, len(d_affine)):
        ...
        if (if_activation[i][1] == 1):
            ...
            relu_compute_GPU(d_lbs, d_ubs, d_relu[:, i], d_active_pattern[:, i], d_l1_lb, d_l1_ub)
        ...
        if (if_activation[i, 1] == 1):  # assuming if first node in a layer has activation then all do
            ...
            for j in range(1, len(d_affine[0])):
                ...
                print(f"Relu eq (LB,UB): {get_bounds_single(ineq_lte[init_id], ineq_gte[init_id], j, l1_lb[init_id], l1_ub[init_id], relu_val=relu[init_id][i][j])}")
        # print stuff
        else:
