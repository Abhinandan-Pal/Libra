...
def back_propagate_l1_CPU(ln_coeff, ineq_prev_lte,
    ...
    l1_lte[:,0] = ln_coeff[:,0]
    l1_gte[:,0] = ln_coeff[:,0]
    for i in range(1, len(l1_lte[0])):
        for k in range(0,len(ln_coeff)):
            for p in range(0, len(ineq_prev_lte[1])):
                if(ln_coeff[k][i]>0):
                    l1_lte[k][p] += ln_coeff[k][i] * ineq_prev_lte[i-1][p]            #should it be i or i-1?
                    l1_gte[k][p] += ln_coeff[k][i] * ineq_prev_gte[i-1][p]

                else:
                    l1_lte[k][p] += ln_coeff[k][i] * ineq_prev_gte[i-1][p]
                    l1_gte[k][p] += ln_coeff[k][i] * ineq_prev_lte[i-1][p]
    return l1_lte, l1_gte
@cuda.jit
def helper(l1_lte,l1_gte,coeff,ineq_prev_lte_i,ineq_prev_gte_i):
    
    k,p = cuda.grid(2)
    if(k>=len(coeff) or k>=len(l1_lte) or k>=len(l1_gte) or p>=len(ineq_prev_lte_i) or p>=len(ineq_prev_gte_i)):
        return
